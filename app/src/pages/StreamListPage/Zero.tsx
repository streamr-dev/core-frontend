    ...
    <EmptyState image={<img src={emptyStateIcon} srcSet={`${emptyStateIcon2x} 2x`} alt="Not found" />}>
        <p>
            ...
            <Message>
                ...
                <a target="_blank" rel="nofollow noopener noreferrer" href="https://corea.streamr.network/">
                    previous network
                </a>{' '}
                and can be migrated.
            </Message>
            <MobileMessage>Use the desktop app to make one.</MobileMessage>
        </p>
    </EmptyState>
)
...
const NoResultsView = ({ onClearFilterClick }) => (
    <EmptyState
        ...
        link={
            <Button kind="special" onClick={onClearFilterClick}>
                Clear filters
            </Button>
        }
    >
        <p>
            ...
            <small>We couldn&apos;t find any streams that match your search.</small>
        </p>
    </EmptyState>
)

export default function Zero({ filter, onClearFilterClick }) {
    return filter ? <NoResultsView onClearFilterClick={onClearFilterClick} /> : <NoCreatedStreamsView />
}
