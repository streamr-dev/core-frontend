...
export default function reducer(state, { type, payload }) {
    switch (type) {
        ...
        case AddField:
            return { ...state, config: { ...state.config, fields: [...state.config.fields, payload] } }
            ...
            return {
                ...
                config: {
                    ...
                    fields: (() => {
                        ...
                        return state.config.fields.map((field) => (field.name === oldName ? { ...field, name: newName } : field))
                    })(),
                },
            }
            ...
            return {
                ...
                config: {
                    ...
                    fields: (() => {
                        ...
                        return state.config.fields.map((field) => (field.name === name ? { ...field, type: newType } : field))
                    })(),
                },
            }

        case DeleteField:
            return { ...state, config: { ...state.config, fields: state.config.fields.filter(({ name }) => name !== payload) } }
