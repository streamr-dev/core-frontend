...
Below is a process which listens to realtime positions of trams from the MQTT API maintained by the Helsinki public transport authority. Incoming events are produced to a Stream in real time.
...
It’s not an optimal way to serve realtime data, because:
