...
You'll see all of your canvases along with their running state on the <Link to={links.userpages.canvases}>canvases page</Link>, inside the Core app. Click on a canvas to open it in the editor. You can also search and sort your canvases by its name, when it was modified, or from it's running state. 
