...
To be precise, when messages are published to a stream, they are actually published to a **partition** within that stream. Partitions can be understood as sub-streams, or in other words, parts of a stream. All streams have at least one partition, and in fact, most streams have just one. One partition is the default, and it is sufficient for streams with moderate rates of data.
