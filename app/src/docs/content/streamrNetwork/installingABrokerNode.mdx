...
```
docker run -it -v $(cd ~/.streamrDocker; pwd):/root/.streamr streamr/broker-node:31.0.0-beta.4 bin/config-wizard
```
...
```
cd ~/.streamrDocker
docker run -it -v ${pwd}:/root/.streamr streamr/broker-node:31.0.0-beta.4 bin/config-wizard
```
...
```
docker run -it -p 7170:7170 -p 7171:7171 -p 1883:1883 -v $(cd ~/.streamrDocker; pwd):/root/.streamr streamr/broker-node:31.0.0-beta.4
```
...
```
cd ~/.streamrDocker
docker run -p 7170:7170 -p 7171:7171 -p 1883:1883 -it -v ${pwd}:/root/.streamr streamr/broker-node:31.0.0-beta.4
```
...
If you don’t have Node.js, install it using [nvm](https://github.com/nvm-sh/nvm#installing-and-updating) or manually from the [Node.js site](https://nodejs.org/en/download/). The Broker requires Node.js version 16.x. Once installed, you can download, configure, and start the Streamr Broker.

### Step 1: Install the latest version using npm

- Run `npm install -g streamr-broker@31.0.0-beta.4` to download and install the package. You may need administrative access to run this command.

```
npm install -g streamr-broker@31.0.0-beta.4
```
