...
```
cd ~/.streamrDocker
docker run -p 7170:7170 -p 7171:7171 -p 1883:1883 -it -v ${pwd}:/root/.streamr streamr/broker-node:31.0.0-beta.3
```
...
- Run `npm install -g streamr-broker@31.0.0-beta.3` to download and install the package. You may need administrative access to run this command.
