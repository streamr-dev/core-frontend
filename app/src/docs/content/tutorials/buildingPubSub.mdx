...
The easiest way to push data to a stream is to use one of our <Link to={docsLinks.sdk}>SDKs</Link>, currently available in <a target="_blank" rel="noopener noreferrer" href="https://github.com/streamr-dev/network-monorepo/tree/main/packages/client">Javascript</a>, <a target="_blank" rel="noopener noreferrer" href="https://github.com/streamr-dev/streamr-client-java">Java</a>. Client libraries for other languages are on the roadmap. Here is an example of publishing data to a stream using Streamr Javascript client.

<CodeSnippet language="javascript" wrapLines showLineNumbers>
    {ClientPub}
</CodeSnippet>
...
<CodeSnippet
    language="text"
    wrapLines
>{`https://streamr.network/api/v1/streams/MY-STREAM-ID/data`}</CodeSnippet>
...
<CodeSnippet
    language="text"
    wrapLines
>{`Authorization: Bearer my-session-token`}</CodeSnippet>
...
Here is the example from the Streamr JavaScript client <a target="_blank" rel="noopener noreferrer" href="https://github.com/streamr-dev/network-monorepo/blob/main/packages/client/README.md">readme</a> on how to subscribe to a stream.
