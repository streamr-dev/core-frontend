...
In addition to the SDKs, there is the <a target="_blank" href={links.external.cliTool}>Command Line Interface (CLI) tool</a> for interacting with Streamr. The CLI tool is a thin wrapper around the <Link to={docsLinks.javascriptSdk}>Streamr Javascript Client</Link>, which does the heavy lifting, while the tool concentrates on CLI concerns: parsing and passing arguments, stdin/stdout, errors, and so forth.
