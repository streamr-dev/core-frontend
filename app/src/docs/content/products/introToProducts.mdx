...
import ProductsCreateFlow2x from './images/intro_to_products_02_mobile@2x.jpg'
...
In the Streamr Marketplace, you can buy and sell a variety of real-time data products, and if you have access to your own real-time data, you can make and sell your own within the Core app. A product is a single stream or bundle of real-time data streams either sold or made available for free on the Marketplace. These streams typically originate from sensors or applications. You can also create crowdsourced data products (sometimes known as data unions) with Core, where they are called Community Products. 

Currently Core and the Marketplace only support crypto token payments using Streamr's native DATA token, an ERC-20 token on the Ethereum blockchain. This means to create or purchase paid products, you will need an Ethereum wallet, some ether for gas, and the DATA token, if making purchases. Below we'll run through the basics of creating a product.

<img
  src={ProductsOverview}
  srcSet={`${ProductsOverview2x} 2x`}
