...
Streamr provides a set of APIs for easy integration with other systems. The APIs cover <Link to={links.docs.api.authentication}>Authentication</Link>, data input, data output, and managing various resources within Streamr (such as streams, canvases, products, and dashboards).

There are <a href="https://en.wikipedia.org/wiki/Representational_state_transfer" target="_blank">REST</a>ful HTTP endpoints that can be accessed using any HTTP library with ease. You can explore these endpoints using the <Link to={links.docs.api.apiExplorer}>API Explorer</Link>. You can expect:
...
If you'd like to contribute a client library and get it listed here, check out our section on <Link to={links.docs.technicalNotes.root}>contributing</Link>.
...
The websocket protocol is easiest to use with one of the available client libraries. If there isn't a client library available for your language, you can dive into the details of the <a href="https://github.com/streamr-dev/streamr-specs/blob/master/PROTOCOL.md" target="_blank">websocket protocol</a>.
