...
This stream listing page displays all streams the current user has a right to publish or subscribe to. Clicking on a stream will open the stream editor. You may only edit streams that you have created or have explicit permission to edit.
...
ENS names allow streams to have human readable names, for example `mydomain.eth/traffic/helsinki`. If you haven’t used an ENS name before, you can easily create one with a small amount of ETH on <Link to={routes.misc.ensDomains()}>ENS Domains</Link>. Your purchased addresses will automatically appear in the stream editor for immediate usage. At a future date, the Stream registry will live on the blockchain.
