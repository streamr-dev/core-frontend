...
When reading from or writing to streams, you need to provide a session token or an API key, or login with an Ethereum account. To read more about API keys or obtaining a session token, refer to the <Link to={links.docs.api.authentication}>Authentication section of the API docs</Link>.
