...
Currently, the symmetric key must be shared between the publisher and the subscribers in a secure way outside of the Streamr Network. In both the <a href="https://github.com/streamr-dev/network-monorepo" target="_blank" rel="noopener noreferrer">Javascript</a> and the <a href="https://github.com/streamr-dev/streamr-client-java" target="_blank" rel="noopener noreferrer">Java</a> SDKs, the publisher can pass group keys per stream as parameters and the subscriber can pass group keys per stream and per publisher as parameters. Later it will be possible to use a secure key-exchange protocol directly in the Streamr Network.
