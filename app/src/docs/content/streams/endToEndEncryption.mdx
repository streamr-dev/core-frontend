...
The subscribers need this symmetric group key in order to decrypt the data. They automatically obtain the key by performing a key exchange with the publisher. The key exchange happens using asymmetric encryption: 
...
To perform key exchange with subscribers, the publisher must be online and present in the network. As the Streamr Network deals with real-time messages, publishers are often constantly online. Especially with old stored data it may happen, however, that the publisher has disappeared since publishing the data, making those messages inaccessible to subscribers who didn't receive the key at the time. This is a consequence of publishers being fully and solely in control of who can access their data on the network.
