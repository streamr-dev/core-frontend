...
The correct secret allows the member to join without manual approval from the Data Union admin. If the secret is omitted, the admin must log in, find pending joinRequests in the Data Union admin view of the product, and approve the requests manually.
