...
Each Data Union has one Admin (owner), and `n` data producing members.
...
The Admins are incentivized to perform this work by the `adminFee`, a fraction of the incoming Data Union revenue. Admins can set the `adminFee`, and members agree to it by joining the Data Union. Previous earnings are not affected by changes in the `adminFee`.
