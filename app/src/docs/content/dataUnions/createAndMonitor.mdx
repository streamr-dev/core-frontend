...
Creating a Data Union product is most easily handled by using the Core app. See <Link to={links.docs.dataUnions.dataUnionsInCore}>Creating a Data Union with Core</Link> for more details. Otherwise, it can be handled this way, but it is somewhat complicated.
...
If the secret is not provided, it will be randomly generated. Note that the request must be authenticated using the Ethereum address of the Administrator of the Data Union, as described in <Link to={links.docs.dataUnions.authAndIdentity}>Data Unions Auth & Identity</Link>.
...
After the DataunionVault is set up, it’s time to create the actual product. This is most easily done using Streamr Core, but can also be done through the <Link to={links.docs.api.root}>HTTP API</Link>.
...
The Streamr Core app also provides monitoring functions for Admins, showing subscriber numbers, total revenue, join requests and more. <Link to={links.docs.dataUnions.dataUnionsInCore}>Create a Data Union with Core</Link> provides more information.
