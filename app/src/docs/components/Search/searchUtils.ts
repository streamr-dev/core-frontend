...
const highlightMatch = (text, startHighlightChar, endHighlightChar) => {
    ...
    textWithHighlight = [text.slice(0, endHighlightChar), '</span>', text.slice(endHighlightChar)].join('')
    textWithHighlight = [
...
export const formatSearchResults = (results: Record<string, any>) => {
    if (!results.length) {
        return []
    }
...
const formatSearchMatchData = (matchData) => {
    ...
    let matchPosition = 0
    let matchLength = 0
    Object.values(matchData).forEach((match: Record<string, any>, matchIndex: number) => {
        // highlighting first match only (for now)
        if (matchIndex === 0) {
            const matches = Object.values(match)
            matches.forEach(({ content }: Record<string, any>, contentIndex) => {
