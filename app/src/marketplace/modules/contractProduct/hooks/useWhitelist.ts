...
export function useWhitelist() {
    ...
    const add = useCallback(
        ({ productId, address, status, isPending }: WhitelistParams) => {
            try {
                const result = update({
                    data: {
                        address,
    ...
    const edit = useCallback(
        ({ address, status, isPending }: WhitelistParams) => {
            try {
                update({
                    data: {
                        address,
    ...
    const remove = useCallback(
        ({ productId, address }: WhitelistParams) => {
            try {
                const result = update({
                    data: {
                        address,
    ...
    return useMemo(
        ...
        [items, load, add, edit, remove, reset],
    )
}
