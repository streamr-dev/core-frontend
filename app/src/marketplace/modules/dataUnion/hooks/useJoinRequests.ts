...
import type { DataUnionId } from '$mp/types/project-types'
...
function useJoinRequests() {
    ...
    const load = useCallback(
        async ({ dataUnionId, filter }: LoadParams) => {
            setFetching(true)
            setError(undefined)
    ...
    const approve = useCallback(
        async ({ dataUnionId, joinRequestId }: UpdateParams) => {
            try {
    ...
    const addRequest = useCallback(
        async ({ dataUnionId, memberAddress }: AddParams) => {
            try {
    ...
    return useMemo(
        ...
        [load, fetching, ids, members, error, approve, addRequest, remove],
    )
}
