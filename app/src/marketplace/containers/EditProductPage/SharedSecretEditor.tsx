...
const UnstyledSharedSecretEditor = ({ disabled, ...props }: Props) => {
    ...
    const { load, isLoading, secrets, edit, add, remove } = useDataUnionSecrets()
    ...
    return (
        <div {...props}>
            {isLoading === null && (
                <LoadButton onClick={() => loadSecrets(dataUnionId)} disabled={isLoading || isDisabled}>
                    Load secrets
                </LoadButton>
            )}
            ...
            {isLoading === false && secrets.length === 0 && <Empty>You haven&apos;t created any secrets yet</Empty>}
            {isLoading === false && (
                <AddKeyFieldWrapper>
                    <AddKeyField
