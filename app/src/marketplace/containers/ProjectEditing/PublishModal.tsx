...
export const PublishOrUnpublishModal = ({ product, api }: Props) => {
    ...
    useEffect(() => {
        if (!productId || !publishRef.current) {
            return
        }
    ...
    useEffect(() => {
        ...
        setWeb3Actions(
            new Set(
                queue
                    .getActions()
                    .filter(({ requireWeb3: isTransaction }) => !!isTransaction)
                    .map(({ id }) => id),
            ),
        )
        queue
    ...
    if (modalError) {
        return <ErrorDialog message={modalError.message} onClose={onClose} />
    }
    ...
    if (!checkingWeb3 && !!requireWeb3 && !!requiredOwner && (!account || !areAddressesEqual(account, requiredOwner))) {
        return (
            <UnlockWalletDialog onClose={onClose} requiredAddress={requiredOwner}>
                <p>Please select the account with address</p>
            </UnlockWalletDialog>
        )
    }
