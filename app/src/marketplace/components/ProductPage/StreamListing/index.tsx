            ...
            <Segment.Header>
                <HeaderRow>
                    <TitleItem>
                        ...
                        {!!locked && <LockedNotice>Subscribe to unlock</LockedNotice>}
                    </TitleItem>
                    ...
                    {streams.length > 0 &&
                        streams.map(({ id: streamId, description }) => (
                            <DataRow key={streamId} locked={locked} clickable={!locked && !!showPreview}>
                                <TitleItem title={description}>{truncate(streamId)}</TitleItem>
                                ...
                                {!!showPreview && <MobileHitTarget onClick={() => onStreamPreview(streamId)} />}
                            </DataRow>
                        ))}
                    {!!fetchingStreams && streams.length === 0 && (
                        <DataRow locked={locked}>
                            <RowItem>Loading streams...</RowItem>
                        </DataRow>
                    )}
                    {!fetchingStreams && streams.length === 0 && (
                        <DataRow locked={locked}>
                            <RowItem>No streams found.</RowItem>
                        </DataRow>
                    )}
                    {!!hasMoreResults && (
                        <LoadMoreRow locked={locked}>
                            <RowItem>
                                <Button kind="special" variant="dark" onClick={onLoadMore} disabled={!!fetchingStreams}>
                                    View more
                                </Button>
                            </RowItem>
                        </LoadMoreRow>
