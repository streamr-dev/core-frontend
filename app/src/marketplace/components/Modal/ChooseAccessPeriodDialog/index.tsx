...
export const ChooseAccessPeriodDialog = ({
    ...
    const setExternalPrices = useDebounced(
        useCallback(
            // TODO add typing
            async ({ priceInToken: inToken, priceInUsd: inUsd, paymentCurrency: currency }: any) => {
                setLoading(true)
                let price
                let usdEstimate
    ...
    const displayPrice = useMemo(() => (new BN(currentPrice).isNaN() ? 'N/A' : new BN(currentPrice).toFixed(2)), [currentPrice])
    const displayApproxUsd = useMemo(() => (new BN(approxUsd).isNaN() ? 'N/A' : new BN(approxUsd).toFixed(2)), [approxUsd])
    useEffect(() => {
        setExternalPrices({
            priceInToken,
            priceInUsd,
