...
const UnstyledNewShareForm = ({ className, onAdd }) => {
    ...
    const validationError = (() => {
        ...
        if (value.toLowerCase() === address0) {
            return 'Can not share to anonymous'
        }
    })()
        ...
        <form className={className} onSubmit={onSubmit}>
            ...
            <Inner>
                <Text
                    ...
                    value={value}
                />
                <Button disabled={!isValid} kind="secondary" type="submit">
                    <SvgIcon name="plus" />
                </Button>
                {showValidationError && <Errors>{validationError}</Errors>}
            </Inner>
        </form>
    )
}
