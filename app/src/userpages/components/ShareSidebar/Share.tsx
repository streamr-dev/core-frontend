...
const UnstyledCollapse = ({ children, open, ...props }) => {
    ...
    return (
        <div {...props} style={open ? style : empty}>
...
const UnstyledShare = ({ className, userId, onSelect, selected }) => {
    ...
    const group = useMemo(() => identifyGroup(resourceType, userCombination), [resourceType, userCombination])
    const isCustom = groups[resourceType][group] !== userCombination
    const onPermissionChange = useCallback(
        (operationKey, enabled) => {
    ...
    return (
        <Fragment>
            <Sidebar.Container
                ...
                tabIndex="0"
            >
                {selected && <DismissBox onClick={onDismiss} />}
                <Header>
                    <div>
                        <h4 title={userId}>
                            {truncate(userId)}
            ...
            {error && <Sidebar.Container as={ErrorMessage}>{error.message}</Sidebar.Container>}
        </Fragment>
    )
}
