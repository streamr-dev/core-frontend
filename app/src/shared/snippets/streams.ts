export const CreateJavascriptClient = `const streamr = new StreamrClient({
    auth: {
...
export const AuthJavascriptClient = `new StreamrClient({
    auth: {
        ethereum: window.ethereum,
    }
})`
export const AuthJavaClient =
    'StreamrClient client = new StreamrClient(new EthereumAuthenticationMethod(myEthereumPrivateKey));'
export const SubscribeJavascriptClient = `// Subscribe to a stream
...
export const SubscribeJavaClient = `Subscription sub = client.subscribe(stream, new MessageHandler() {
    @Override
...
export const PublishJavascriptClient = `// Here is the message we'll be sending
const msg = {
    hello: 'world',
    random: Math.random()
}
...
export const PublishJavaClient = `// Create the message payload, which is represented as a Map
