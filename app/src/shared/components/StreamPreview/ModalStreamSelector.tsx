...
export const ModalStreamSelector: FunctionComponent<StreamSelectorProps> = (props) => {
    ...
    return <div>
        <SelectorWrapper>
            <StreamName onClick={openModal}>{props.selectedStream && truncateStreamName(props.selectedStream)}</StreamName>
        </SelectorWrapper>
        {isOpen && <StreamSelectorModal api={api} {...props} />}
    </div>
}

const StreamSelectorModal: FunctionComponent<StreamSelectorModalProps> = ({api, streamIds, selectedStream, onChange}) => {
        ...
        <ModalDialog onClose={onClose} fullpage noScroll={false} closeOnEsc={true}>
            <ModalContainer>
                <Header>
                    ...
                    <Button onClick={onClose} kind={'secondary'}>Done</Button>
                </Header>
                <StreamsList>
                    {streamIds.map((streamId, index) => {
                        return <li
                            key={index}
                            className={streamId === selectedStream ? 'active' : ''}
                            onClick={() => {
                                onChange(streamId)
                                onClose()
                            }}
                        >
                            ...
                            {streamId === selectedStream && <SvgIcon name={'checkMarkMono'}/>}
                        </li>
                    })}
                </StreamsList>
            </ModalContainer>
