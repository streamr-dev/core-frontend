...
const renderLabels = (segments: Array<Segment>, disabled: boolean = false) =>
    segments.map((segment, index) => (
        ...
        >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 10 10" className={styles.labelCircle}>
                <circle
                    cx="5"
                    cy="5"
                    r={4}
...
const renderSegments = (segments: Array<DonutSegment>, strokeWidth: number) =>
    segments.map((segment, index) => (
        <circle // eslint-disable-next-line react/no-array-index-key
            key={index}
            cx="50"
            cy="50"
            ...
            strokeDasharray={`${segment.percentage * CIRCLE_CIRCUMFERENCE} ${
                (1 - segment.percentage) * CIRCLE_CIRCUMFERENCE
            }`}
...
const DonutChart = ({ className, chartClassName, strokeWidth, data, disabled }: Props) => {
    ...
    const renderedSegments = useMemo(() => renderSegments(segments, strokeWidth), [segments, strokeWidth])
    const total = useMemo(() => {
        if (disabled) {
            return 0
        }
    ...
    return (
        <div className={cx(styles.root, className)}>
            ...
            <div className={styles.labelContainer}>{renderedLabels}</div>
        </div>
    )
}
