...
const BusLine: FunctionComponent<{dynamicScrollPosition: any, children: ReactNode | ReactNode[]}> = ({ children = null, dynamicScrollPosition }) => {
    ...
    useEffect(() => {
        ...
        const touchScroll = () => {
            ...
            if (document.body && isMounted() && (wheel || scrollbar)) {
                ...
                const guideY =
                    scrollY +
                    (dynamicScrollPosition ? Math.floor(windowHeight * (scrollY / (documentHeight - windowHeight))) : 0)
    ...
    const value = useMemo(
        ...
        [defaultStop, link, stop, unlink],
    )
    return <BusLineContext.Provider value={value}>{children}</BusLineContext.Provider>
}
