...
const RadioButtonGroup = ({ name, options, selectedOption, className, onChange, disabled = false }: Props) => {
    ...
    const activeWidth = useMemo(() => (options.length ? 100 / options.length : 0), [options])
    const activeMargin = useMemo(() => Math.max(0, options.indexOf(selection)), [options, selection])
    return (
        <div
