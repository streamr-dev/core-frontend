...
while true; do http_code=$(curl -s -o /dev/null -I -w "%{http_code}" http://localhost:8081/streamr-core/login/auth); if [ $http_code = 200 ]; then echo "EE up and running"; docker logs streamr_dev_engine-and-editor; break; else echo "EE not receiving connections"; sleep 5s; fi; done
