...
function reducer(state, { type, value, detectUnit = false }) {
    switch (type) {
        case Invalidate:
            return { ...state, cache: (state.cache || 0) + 1 }
            ...
            return (() => {
                ...
                if (detectUnit) {
                    ...
                    for (const [unit, mx] of units) {
                        if (value % mx === 0) {
                            return { ...state, quantity: value / mx, unit }
                        }
                    }
                } else {
                    // eslint-disable-next-line no-restricted-syntax
                ...
                return { ...state, quantity: value, unit: nativeUnit }
            })()

        case SetQuantity:
            return { ...state, quantity: Number.isNaN(value) ? undefined : value }
